...
class Get_new_points_function:
    ...
    def get_new_bb_vector(self, number, param={"p": 0.5}):
        ...
        while number > 0:
            new_point = np.random.choice(
                [0, 1],
                size=(l + g - 1) * 2,
                p=[1 - density_expectation, density_expectation],
            )
...
class QEC_Dataset(Dataset):
    def __init__(
        ...
        self.gnp = Get_new_points_function(
            method="bb", hyperparameters=para_dict, param=gnp_param
        ...
        def objectivefunction(code):
            if noise_model == "depolarizing":
                ...
                custom_error_model = noise_and_decoder_param.get(
                    "custom_error_model", {}
                )
