...
class Get_new_points_function:
    ...
    def get_new_points_HGP(self, number):
        return np.random.randint(
            0,
            code_constructor.para_dict["m"] + 1,
            (number, code_constructor.para_dict["p"] * code_constructor.para_dict["q"]),
        )
...
if __name__ == "__main__":
    ...
    flat3 = y_init + flat3
    with open(
        f"./data/BO_results/RS_{l}_{g}_{dataset_index}_{seed}_{lambda_}.pkl", "wb"
    ) as f:
        results = {"best_x": best_x, "best_y": best_y, "evaluation_history": flat3}
