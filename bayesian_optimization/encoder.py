...
class CSSEncoder:
    def __init__(
        self, info, mode="graph", grakel_use=False, logical=False, cc_patern=[]
    ):
        """
        ...
        if mode == "graph":
            ...
            self.encoder = GraphEncoder(
                n, nx, nz, grakel_use=grakel_use, logical=logical
            )
    ...
    def encode(self, hx, hz, lx=None, lz=None):
        ...
        if self.logical == False:
            adjacency_matrix = np.zeros(
                (self.n + self.nx + self.nz, self.n + self.nx + self.nz)
            )
            ...
            if not self.grakel_use:
                ...
                # print(adjacency_matrix)
                g = Graph(
                    initialization_object=adjacency_matrix,
            ...
            for i in range(self.n):
                ...
                for j in range(lz.shape[0]):
                    if lz[j, i] == 1:
                        adjacency_matrix[i][
                            self.n + self.nx + self.nz + lx.shape[0] + j
                        ] = 1
                        adjacency_matrix[self.n + self.nx + self.nz + lx.shape[0] + j][
                            i
                        ] = 1
            if not self.grakel_use:
                ...
                node_onehots[
                    self.n + self.nx + self.nz + lx.shape[0] : total_nodes, 4
                ] = 1.0
                ...
                for i in range(self.n + self.nx, self.n + self.nx + self.nz):
                    node_labels[i] = "-"
                for i in range(
                    self.n + self.nx + self.nz, self.n + self.nx + self.nz + lx.shape[0]
                ):
                    node_labels[i] = "x"
                for i in range(
                    self.n + self.nx + self.nz + lx.shape[0],
                    self.n + self.nx + self.nz + lx.shape[0] + lz.shape[0],
                ):
                    node_labels[i] = "z"
...
def _vstack_bool_int(*rows: np.ndarray) -> np.ndarray:
    ...
    if not rows_b:
        return np.zeros(
            (0, rows[0].shape[1]), dtype=np.int64
        )  # assume widths match if any
...
def _vstack_bool_int(*rows: np.ndarray) -> np.ndarray:
    ...
    for rb in rows_b:
        assert rb.shape[1] == width, (
            "All stacked matrices must have the same number of columns"
        )
    return np.vstack(rows_b).astype(np.int64, copy=False)


def _np_to_sparse_coo(
    M: np.ndarray, device: Optional[torch.device], dtype: torch.dtype
) -> torch.Tensor:
    """
...
class RelationEncoder:
    ...
    def _build_relation_matrix(
        self, tag: str, prim_np: Dict[Tuple[str, str], np.ndarray]
        ...
        if raw.startswith("adj"):
            raise NotImplementedError(
                f"Relation '{tag}': 'adj*' builders are reserved for later."
            )
    ...
    def _lookup_primitive(
        src: str, dst: str, prim_np: Dict[Tuple[str, str], np.ndarray], tag: str
    ) -> np.ndarray:
        key = (src, dst)
        if key not in prim_np:
            raise ValueError(
                f"Relation '{tag}': unsupported primitive mapping '{src}->{dst}'."
            )
        return prim_np[key]
