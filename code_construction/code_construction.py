...
class CSSCode(StabilizerCode):
    def __init__(self, hx=None, hz=None) -> None:
        self.hx = np.array(hx)
        self.hz = np.array(hz)
        self.h = np.block([[hz, np.zeros(hz.shape)], [np.zeros(hx.shape), hx]])
        if not self.check_validity():
...
class CodeConstructor:
    ...
    def check_parameters_validity(self):
        ...
        #     return self.check_bivariate_bycicle_parameters_validity()
            ...
            if "g" in self.para_dict.keys():
                ...
                if type(self.para_dict["ga"]) == type(int(1)) and type(
                    self.para_dict["gb"]
                ) == type(int(1)):
                    return True
            return False
        ...
        if "p_2" in self.para_dict.keys():
            ...
            self.n = (
                ...
                * (
                    ...
                    + self.para_dict["q"] * self.para_dict["q_2"]
                )
            )
            self.nx = (
                self.para_dict["m"]
            ...
            self.n = self.para_dict["m"] ** 2 * (
                self.para_dict["p"] ** 2 + self.para_dict["q"] ** 2
            )
            self.nx = (
                self.para_dict["m"] ** 2 * self.para_dict["p"] * self.para_dict["q"]
            )
            self.nz = self.nx
        ...
        for i in range(p + 1):
            for j in range((p - 1) // 2):
                if index(i - 1, j + (i + 1) % 2) != -1:
                    x_stabilizers[i * (p - 1) // 2 + j].append(
                        index(i - 1, 2 * j + (i + 1) % 2)
                    )
                if index(i - 1, j + (i + 1) % 2 + 1) != -1:
                    x_stabilizers[i * (p - 1) // 2 + j].append(
                        index(i - 1, 2 * j + (i + 1) % 2 + 1)
                    )
        ...
        for i in range(p + 1):
            for j in range((p - 1) // 2):
                if index(i - 1, j + (i) % 2, reversed=True) != -1:
                    z_stabilizers[i * (p - 1) // 2 + j].append(
                        index(i - 1, 2 * j + (i) % 2, reversed=True)
                    )
                if index(i - 1, j + (i) % 2 + 1, reversed=True) != -1:
                    z_stabilizers[i * (p - 1) // 2 + j].append(
                        index(i - 1, 2 * j + (i) % 2 + 1, reversed=True)
                    )
                if index(i, j + (i) % 2, reversed=True) != -1:
                    z_stabilizers[i * (p - 1) // 2 + j].append(
                        index(i, 2 * j + (i) % 2, reversed=True)
                    )
                if index(i, j + (i) % 2 + 1, reversed=True) != -1:
                    z_stabilizers[i * (p - 1) // 2 + j].append(
                        index(i, 2 * j + (i) % 2 + 1, reversed=True)
                    )
    ...
    def canonical_construction(self, bitstring, CSS=False) -> StabilizerCode:
        ...
        if CSS == False:
            ...
            D = (A_1.T + C @ A_2.T) % 2
            hx = np.block([D, np.eye(s), C])
            hz = np.block([np.eye(r), A_1, A_2])
            return CSSCode(hx, hz)
    ...
    def qc_ldpc_hgp_construction(self, M, M_2=None, form=None) -> CSSCode:
        ...
        """
        if M_2 is None:
            H1 = self.ldpc_construction(
                self.para_dict["p"], self.para_dict["q"], self.para_dict["m"], M
            )
            H2 = self.ldpc_construction(
                self.para_dict["p"], self.para_dict["q"], self.para_dict["m"], M
            )
        else:
            H1 = self.ldpc_construction(
            ...
            H2 = self.ldpc_construction(
                self.para_dict["p_2"], self.para_dict["q_2"], self.para_dict["m_2"], M_2
            )
        r1, n1 = H1.shape
        r2, n2 = H2.shape
        # HX = [H1 ⊗ In2 | Ir1 ⊗ H2.T]
